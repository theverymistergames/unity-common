...
namespace MisterGames.BlueprintLib {
    ...
    public class BlueprintSourceTweenProgressActionsCombine :
        ...
        BlueprintSources.IOutput<BlueprintNodeTweenProgressActionsCombine, ITweenProgressCallback>,
        BlueprintSources.ICloneable {}

    [Serializable]
    [BlueprintNode(Name = "Combine Tween Progress Actions", Category = "Tweens/Actions", Color = BlueprintColors.Node.Actions)]
    public struct BlueprintNodeTweenProgressActionsCombine : IBlueprintNode, IBlueprintOutput<ITweenProgressCallback>  {

        public void CreatePorts(IBlueprintMeta meta, NodeId id) {
            meta.AddPort(id, Port.Input<ITweenProgressCallback>("Actions").Capacity(PortCapacity.Multiple));
        ...
        public ITweenProgressCallback GetPortValue(IBlueprint blueprint, NodeToken token, int port) {
            ...
            var action = links.Read<ITweenProgressCallback>();
            if (!links.MoveNext()) return action;

            var result = new TweenProgressActions { actions = new List<ITweenProgressCallback>() };
            if (action != null) result.actions.Add(action);

            action = links.Read<ITweenProgressCallback>();
            if (action != null) result.actions.Add(action);

            while (links.MoveNext()) {
                if (links.Read<ITweenProgressCallback>() is {} v) {
                    result.actions.Add(v);
                    continue;
                }

                if (links.Read<ITweenProgressCallback[]>() is {} actions) {
