...
namespace MisterGames.Common.Editor.Search {
    ...
    public sealed class ScriptableObjectSearchWindow : EditorWindow {
        ...
        private sealed class TabTargetsState : ScriptableObject {
            public List<ScriptableObject> targets = new();
        }
        ...
        private sealed class TabRuntime {
            public readonly List<ScriptableObject> GroupTargets = new();
        ...
        private const string SearchFoldersPrefsKeyPrefix = "ScriptableObjectSearchWindow.SearchFolders.";
        private static string SearchFoldersPrefsKey => SearchFoldersPrefsKeyPrefix + Application.dataPath;

        private const string SearchStatePrefsKeyPrefix = "ScriptableObjectSearchWindow.SearchState.";
        ...
        private ScriptableObject[] _requestedSearch;
        ...
        [MenuItem("MisterGames/Tools/Scriptable Object Search")]
        public static void Open() {
            var window = GetWindow<ScriptableObjectSearchWindow>("Scriptable Object Search");
            SetupSize(window);
        }

        [MenuItem("Assets/Search usages", false, 39)]
        private static void SearchUsagesOfSelection() {
            var targets = GetSelectedScriptableObjects();
            if (targets.Length > 0) SearchScriptableObjects(targets);
        }

        [MenuItem("Assets/Search usages", true, 39)]
        private static bool SearchUsagesOfSelectionValidate() {
            return GetSelectedScriptableObjects().Length > 0;
        }

        public static void SearchScriptableObject(ScriptableObject scriptableObject) {
            SearchScriptableObjects(new[] { scriptableObject });
        }

        public static void SearchScriptableObjects(ScriptableObject[] scriptableObjects) {
            var window = GetWindow<ScriptableObjectSearchWindow>("Scriptable Object Search");
            SetupSize(window);
            window._requestedSearch = scriptableObjects;
            window.ApplyRequestedSearchToActiveTab();
        }

        private static ScriptableObject[] GetSelectedScriptableObjects() {
            var objects = Selection.objects;
            if (objects == null || objects.Length == 0) return Array.Empty<ScriptableObject>();

            var result = new List<ScriptableObject>();
            foreach (var obj in objects) {
                if (obj is ScriptableObject so && AssetDatabase.Contains(so)) result.Add(so);
            }

            return result.ToArray();
        }

        private static void SetupSize(ScriptableObjectSearchWindow window) {
        ...
        private void OnEnable() {
            ...
            foreach (var tab in _tabs) {
                ...
                foreach (string targetId in tab.TargetIds) {
                    var target = ResolveTarget(targetId);
                    if (target != null) runtime.TargetsState.targets.Add(target);
                }

                foreach (var group in tab.Groups) {
                    runtime.GroupTargets.Add(ResolveTarget(group.TargetId));
                }
        ...
        private void DrawResults() {
            ...
            for (int g = 0; g < tab.Groups.Count; g++) {
                ...
                DrawGroupHeader(rt, g, group.Label);
        ...
        private void DrawActiveTabTargets(TabRuntime rt) {
            var so = rt.TargetsSerializedObject;
            if (so == null) return;
        ...
        private static void DrawGroupHeader(TabRuntime rt, int groupIndex, string fallbackLabel) {
            if (groupIndex >= rt.GroupTargets.Count) return;
            ...
            var scriptRect = new Rect(totalRect.x + half + gap, totalRect.y, half, totalRect.height);

            var target = rt.GroupTargets[groupIndex];

            if (target == null) {
                EditorGUI.LabelField(searchValueRect, "Search object", fallbackLabel);
                return;
            }

            var script = ResolveOwnerScript(target.GetType().FullName);

            using (new EditorGUI.DisabledScope(true)) {
                EditorGUI.ObjectField(searchValueRect, "Search object", target, typeof(ScriptableObject), allowSceneObjects: false);
                EditorGUI.ObjectField(scriptRect, GUIContent.none, script, typeof(MonoScript), allowSceneObjects: false);
            }
        }
        ...
        private void Search() {
            ...
            activeRt.GroupTargets.AddRange(groupTargets);

            activeTab.Label = BuildTabLabel(activeRt.TargetsState.targets);
        ...
        private void SyncTabTargets(int tabIndex) {
            ...
            foreach (var t in rt.TargetsState.targets) {
                if (t == null) continue;
                tab.TargetIds.Add(GetTargetId(t));
            }
        }
        ...
        private void ApplyRequestedSearchToActiveTab() {
            ...
            foreach (var target in _requestedSearch) {
                if (target != null) rt.TargetsState.targets.Add(target);
            }
        ...
        private static string GetTargetId(ScriptableObject target) {
            return target == null ? string.Empty : GlobalObjectId.GetGlobalObjectIdSlow(target).ToString();
        }

        private static ScriptableObject ResolveTarget(string targetId) {
            if (string.IsNullOrEmpty(targetId) || !GlobalObjectId.TryParse(targetId, out var globalObjectId)) return null;
            return GlobalObjectId.GlobalObjectIdentifierToObjectSlow(globalObjectId) as ScriptableObject;
        }

        private static string GetTargetLabel(ScriptableObject target) {
            return target == null ? "<null>" : $"{target.name} ({target.GetType().Name})";
        }

        private static string BuildTabLabel(List<ScriptableObject> targets) {
            int validCount = 0;
            string first = null;
