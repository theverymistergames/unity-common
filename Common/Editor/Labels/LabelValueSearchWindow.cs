...
namespace MisterGames.Common.Editor.Labels {
    ...
    public sealed class LabelValueSearchWindow : EditorWindow {
        ...
        private void Search() {
            ...
            foreach (var (lib, ids) in libraryGroups) {
                ...
                bool Predicate(SerializedReferenceUsageFinder.UsageCandidate<LabelLibraryBase> u) {
                    if (!IsLabelValue(u, lib, ids, out int id)) return false;
            ...
            FieldInfo fi = _targetsProperty.GetFieldInfo();
            ...
            foreach (var target in _state.targets) {
                ...
                AddGroupHeaderToRuntime(runtime, target.library, target.id);
            }
        ...
        private void AddGroupHeaderToRuntime(TabRuntime runtime, LabelLibraryBase lib, int id) {
            ...
            if (lib != null) state.label = new LabelValue(lib, id);
            var so = new SerializedObject(state);
            var labelProp = so.FindProperty(nameof(SingleLabelState.label));
            ...
            runtime.GroupLibraryProperties.Add(labelProp.FindPropertyRelative("library"));
        }
        ...
        private void SaveSearchState() {
            ...
            foreach (var t in _state.targets) {
                if (t.library == null) continue;
                saved.TargetLibraryPaths.Add(AssetDatabase.GetAssetPath(t.library));
                saved.TargetIds.Add(t.id);
            }
        ...
        private void LoadSearchState() {
            ...
            _state.targets.Clear();
            for (int i = 0; i < saved.TargetLibraryPaths.Count && i < saved.TargetIds.Count; i++) {
                var lib = AssetDatabase.LoadAssetAtPath<LabelLibraryBase>(saved.TargetLibraryPaths[i]);
                if (lib != null) _state.targets.Add(new LabelValue(lib, saved.TargetIds[i]));
            }
        }
