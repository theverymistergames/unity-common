...
namespace MisterGames.Common.Data {
    ...
    [Serializable]
    public sealed class SerializedStructMap<K, V>
        where K : struct
        where V : struct
    {
        ...
        private struct Entry {
            ...
            public override string ToString() {
                return $"Entry(key {key}, {(hasValue ? $"value {value}" : "no value")})";
            }
        }

        public int Count => _indexMap.Count;

        private static V DefaultValue = default;
        private readonly Queue<int> _freeIndices = new Queue<int>();

        public SerializedStructMap(int capacity = 0) {
            _entries = new Entry[capacity];
            _indexMap = new SerializedDictionary<K, int>(capacity);
        }

        public ref V Get(K key) {
            if (!_indexMap.TryGetValue(key, out int index)) return ref DefaultValue;
        ...
        public override string ToString() {
            ...
            sb.AppendLine($"{nameof(SerializedStructMap<K, V>)}(values {_indexMap.Count}/{_entries.Length}):");
