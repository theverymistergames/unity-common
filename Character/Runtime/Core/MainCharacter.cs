...
namespace MisterGames.Character.Core {
    ...
    public sealed class MainCharacter : MonoBehaviour, IActorComponent {
        ...
        void IActorComponent.OnAwake(IActor actor) {
            if (actor == CharacterSpawner.Instance.GetCharacter()) return;
            ...
            Debug.LogError(
                ...
                $"only original {nameof(MainCharacter)} at {CharacterSpawner.Instance.GetCharacter().Transform.GetPathInScene(includeSceneName: true)} should exist. " + 
            ...
            CharacterSpawner.Instance.GetCharacter().GetComponent<CharacterMotionPipeline>().Teleport(pos, rot);
