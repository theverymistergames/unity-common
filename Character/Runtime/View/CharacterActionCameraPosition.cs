    ...
    public sealed class CharacterActionCameraPosition : IAsyncAction, IDependency {
        ...
        public async UniTask Apply(object source, CancellationToken cancellationToken = default) {
            ...
            while (!cancellationToken.IsCancellationRequested) {
                ...
                if (x.HasValue) position.x += x.Value.multiplier * x.Value.curve.Evaluate(progress);
                if (y.HasValue) position.y += y.Value.multiplier * y.Value.curve.Evaluate(progress);
                if (z.HasValue) position.z += z.Value.multiplier * z.Value.curve.Evaluate(progress);
