...
namespace MisterGames.Blueprints.Validation {
    ...
    internal static class PortValidator {
        ...
        public static bool ValidateExternalPortWithExistingSignature(BlueprintMeta meta, Port port) {
        ...
        public static bool ValidatePort(BlueprintMeta meta, NodeId id, int index) {
        ...
        private static bool ValidateEnterPort(BlueprintMeta meta, NodeId id, Port port, int index) {
            var source = meta.GetNodeSource(id);

            if (source is not (IBlueprintEnter or IBlueprintInternalLink)) {
                Debug.LogError($"Blueprint `{meta.owner}`: " +
        ...
        private static bool ValidateDynamicOutputPort(BlueprintMeta meta, NodeId id, Port port, int index) {
            var source = meta.GetNodeSource(id);

            if (source is not (IBlueprintOutput or IBlueprintInternalLink)) {
                Debug.LogError($"Blueprint `{meta.owner}`: " +
        ...
        private static bool ValidateOutputPort(BlueprintMeta meta, NodeId id, Port port, int index) {
            var source = meta.GetNodeSource(id);

            if (source is not (IBlueprintOutput or IBlueprintInternalLink) &&
                ValidationUtils.GetGenericInterface(source.GetType(), typeof(IBlueprintOutput<>), port.DataType) == null
            ) {
                Debug.LogError($"Blueprint `{meta.owner}`: " +
