    ...
    internal static class BlueprintValidation {
        ...
        public static bool ArePortsCompatible(Port a, Port b) {
            ...
            return a.mode switch {
                ...
                Port.Mode.InputArray => b.mode is (Port.Mode.Output or Port.Mode.OutputArray) && a.DataType == b.DataType || b.mode == Port.Mode.NonTypedOutput,
                Port.Mode.OutputArray => b.mode == Port.Mode.InputArray && a.DataType == b.DataType || b.mode == Port.Mode.NonTypedInput,
                Port.Mode.NonTypedInput => b.mode is Port.Mode.Output or Port.Mode.OutputArray or Port.Mode.NonTypedOutput,
                Port.Mode.NonTypedOutput => b.mode is Port.Mode.Input or Port.Mode.InputArray or Port.Mode.NonTypedInput,
        ...
        public static bool ValidatePort(BlueprintAsset asset, BlueprintNodeMeta nodeMeta, int portIndex) {
            ...
            return port.mode switch {
                ...
                Port.Mode.OutputArray => ValidateOutputArrayPort(asset, port.DataType, nodeMeta, portIndex),
        ...
        public static bool ValidateExternalPortWithExistingSignature(BlueprintAsset asset, Port port) {
            if (port.mode is Port.Mode.Output or Port.Mode.OutputArray or Port.Mode.NonTypedOutput) {
        ...
        private static bool ValidateLinkToInputPort(BlueprintAsset asset, Type dataType, BlueprintNodeMeta nodeMeta, int portIndex) {
            ...
            var node = nodeMeta.Node;
            if (port.mode == Port.Mode.NonTypedOutput && node is not IBlueprintPortLinker) {
                Debug.LogError($"Blueprint `{asset.name}`: " +
        ...
        private static bool ValidateLinkToInputArrayPort(BlueprintAsset asset, Type dataType, BlueprintNodeMeta nodeMeta, int portIndex) {
            ...
            var port = nodeMeta.Ports[portIndex];
            if (port.mode is not (Port.Mode.Output or Port.Mode.OutputArray or Port.Mode.NonTypedOutput)) {
                Debug.LogError($"Blueprint `{asset.name}`: " +
                               $"Validation failed for output port {portIndex} of node {nodeMeta}: " +
                               $"this port is not an output port.");
                return false;
            ...
            if (port.mode == Port.Mode.OutputArray && port.DataType != dataType) {
                Debug.LogError($"Blueprint `{asset.name}`: " +
                               $"Validation failed for output port {portIndex} of node {nodeMeta}: " +
                               $"port type is not {dataType.Name}.");
                return false;
            ...
            if (port.mode == Port.Mode.OutputArray &&
                !HasGenericInterface(node.GetType(), typeof(IBlueprintOutputArray<>), port.DataType)
               ) {
                Debug.LogError($"Blueprint `{asset.name}`: " +
                               $"Validation failed for output port {portIndex} of node {nodeMeta}: " +
                               $"node class {node.GetType().Name} does not implement " +
                               $"interface {typeof(IBlueprintOutputArray<>).Name}<{port.DataType.Name}>.");
                return false;
            }
        ...
        private static bool ValidateOutputArrayPort(BlueprintAsset asset, Type dataType, BlueprintNodeMeta nodeMeta, int portIndex) {
            if (portIndex < 0 || portIndex > nodeMeta.Ports.Length - 1) {
                Debug.LogError($"Blueprint `{asset.name}`: " +
                               $"Validation failed for output port {portIndex} of node {nodeMeta}: " +
                               $"{nodeMeta} has no port with index {portIndex}.");
            ...
            var node = nodeMeta.Node;
            if (!HasGenericInterface(node.GetType(), typeof(IBlueprintOutputArray<>), port.DataType)
               ) {
                Debug.LogError($"Blueprint `{asset.name}`: " +
                               $"Validation failed for output port {portIndex} of node {nodeMeta}: " +
                               $"node class {node.GetType().Name} does not implement " +
