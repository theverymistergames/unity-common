...
namespace MisterGames.Blueprints.Validation {
    ...
    internal static class LinkValidator {
        ...
        public static bool ValidateInternalLink(BlueprintMeta meta, NodeId id, int port, IBlueprintInternalLink link) {
            int portCount = meta.GetPortCount(id);
            if (port < 0 || port >= portCount) {
                Debug.LogError($"Blueprint `{meta.owner}`: " +
        ...
        public static bool ValidateHashLink(IBlueprintHashLink link, BlueprintMeta meta, NodeId id) {
        ...
        public static bool ValidateNodeLink(BlueprintMeta meta, NodeId id, int port, NodeId toId, int toPort) {
        ...
        private static bool ValidateLinkFromEnterPort(BlueprintMeta meta, NodeId id, int port, NodeId toId, int toPort) {
            Debug.LogError($"Blueprint `{meta.owner}`: " +
                           $"Validation failed for link [node {id}, port {port} :: node {toId}, port {toPort}]: " +
        ...
        private static bool ValidateLinkFromExitPort(BlueprintMeta meta, NodeId id, int port, NodeId toId, int toPort) {
        ...
        private static bool ValidateLinkFromDynamicInputPort(BlueprintMeta meta, NodeId id, int port, NodeId toId, int toPort) {
            if (toPort < 0 || toPort > meta.GetPortCount(toId) - 1) {
                Debug.LogError($"Blueprint `{meta.owner}`: " +
                               $"Validation failed for link [node {id}, port {port} :: node {toId}, port {toPort}]: " +
                               $"node {toId} has no port with index {toPort}.");
        ...
        private static bool ValidateLinkFromDynamicOutputPort(BlueprintMeta meta, NodeId id, int port, NodeId toId, int toPort) {
            Debug.LogError($"Blueprint `{meta.owner}`: " +
                           $"Validation failed for link [node {id}, port {port} :: node {toId}, port {toPort}]: " +
        ...
        private static bool ValidateLinkFromInputPort(BlueprintMeta meta, NodeId id, int port, NodeId toId, int toPort) {
            if (toPort < 0 || toPort > meta.GetPortCount(toId) - 1) {
                Debug.LogError($"Blueprint `{meta.owner}`: " +
                               $"Validation failed for link [node {id}, port {port} :: node {toId}, port {toPort}]: " +
                               $"node {toId} has no port with index {toPort}.");
        ...
        private static bool ValidateLinkFromOutputPort(BlueprintMeta meta, NodeId id, int port, NodeId toId, int toPort) {
            Debug.LogError($"Blueprint `{meta.owner}`: " +
                           $"Validation failed for link [node {id}, port {port} :: node {toId}, port {toPort}]: " +
