...
namespace MisterGames.Blueprints.Runtime {
    ...
    internal sealed class BlueprintCompiler {
        ...
        public RuntimeBlueprint Compile(BlueprintMeta meta, IBlueprintFactory factory, IBlueprintHost host) {
            ...
            var blueprint = new RuntimeBlueprint(root, factory, nodeStorage, linkStorage, blackboardStorage);
        ...
        public void CompileSubgraph(BlueprintMeta meta, SubgraphCompileData data) {
            _runtimeNodeMap.Clear();
            _hashLinks.Clear();
        ...
        private void CompileNodes(
            ...
            foreach (var id in nodes) {
                ...
                for (int p = 0; p < portCount; p++) {
                    ...
                    PortValidator.ValidatePort(meta, id, p);
#endif
                    ...
                    if (isExternal) {
                        ...
                        LinkValidator.ValidateRootLink(meta, linkStorage, id, p, root, sign);
#endif
...
#if DEVELOPMENT_BUILD || UNITY_EDITOR
                            LinkValidator.ValidateInternalLink(meta, id, p, internalLink);
#endif
...
#if DEVELOPMENT_BUILD || UNITY_EDITOR
                        LinkValidator.ValidateNodeLink(meta, id, p, linkedId, link.port);
#endif
                ...
                if (source is IBlueprintHashLink hashLink && hashLink.TryGetLinkedPort(id, out int hash, out int linkedPort)) {
#if DEVELOPMENT_BUILD || UNITY_EDITOR
                    LinkValidator.ValidateHashLink(hashLink, meta, id);
#endif
                    ...
                    _hashLinks.AddEndPoint(dirRoot, new RuntimeLink(runtimeId.source, runtimeId.node, linkedPort));
                }
